...
export const paymentService = {
  ...
  async startCheckout(plan: PlanId, returnUrl?: string): Promise<{ url: string | null; simulated: boolean; sessionId: string | null }> {
    const { data } = await apiClient.post<{ url: string | null; simulated: boolean; sessionId: string | null }>('/payments/checkout', { plan, returnUrl });
    return data;
  },
